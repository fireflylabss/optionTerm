...
optionTerm is a single surface (GTK4 / libadwaita desktop app). Changelog headings use the channel suffix, e.g. `## [0.2.1-stable] - YYYY-MM-DD`.

`Cargo.toml` / git tags keep the numeric version (`0.2.1`, `v0.2.1`) — the channel lives in the changelog (and packaging notes), matching optionMusic’s CLI vs channel-labeled releases.
...
| **stable** | `0.2.1-stable` | Production-ready: finished for that version, few or no known bugs. |
